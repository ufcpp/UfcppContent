...
```csharp {title="? [] * も付けれない" error-ranges="1:25-1:26,2:25-2:27,3:25-3:26"}
...
```csharp {title="タプルのエイリアスを作りたいという要望は頻出" error-ranges="1:14-1:26,1:28-1:41,2:14-2:17,2:19-2:25,3:13-3:34"}
...
```csharp {title="ジェネリックなエイリアス" error-ranges="1:7-1:14,2:7-2:14,3:7-3:14"}
using Fix2<T> = (T, T);
using Fix3<T> = (T, T, T);
...
```csharp {title="選択肢1: エイリアス自体に where 制約" error-ranges="1:30-1:31"}
...
```csharp {title="エイリアスの時点では素通し" error-ranges="4:17-4:23"}
