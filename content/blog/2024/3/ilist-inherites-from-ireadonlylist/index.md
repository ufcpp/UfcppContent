...
```csharp {title="書き込みがあると共変にできない" error-ranges="2:21-2:24" error-diagnostics="CS0266@2:21-2:24"}
...
```csharp {title="ダイアモンド継承問題" error-ranges="18:11-18:13" error-diagnostics="CS8705@18:11-18:13"}
interface IA
{
    int M();
}
...
```csharp {title="IReadOnlyCollection.Count でダイアモンド継承問題を踏む例" error-ranges="11:11-11:22" error-diagnostics="CS8705@11:11-11:22"}
// corelib とは別のプロジェクトで、別の開発者が保守
// anotherlib.dll
