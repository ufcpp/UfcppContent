...
```csharp {title="stackalloc は基本的にはポインター" error-ranges="18:9-18:26" error-diagnostics="CS0214@18:9-18:26"}
unsafe
{
    // stackalloc の昔からの用法。
...
```csharp {title="() を付ければ safe" error-ranges="3:9-3:26" error-diagnostics="CS0214@3:9-3:26"}
...
```csharp {title="ReadOnlySpan の拡張メソッドに対しては使えない" error-text="stackalloc byte[4]" error-diagnostics="CS1929@6:2-6:20"}
using System.Text;
