...
```csharp {title="拡張メソッド呼びできない例: ユーザー定義の型変換" error-ranges="7:1-7:3,8:2-8:20,9:1-9:13" error-diagnostics="CS1929@7:1-7:3,CS1929@8:2-8:20,CS1929@9:1-9:13"}
// 通常のメソッドとしてなら呼べる。
...
```csharp {error-ranges="7:1-7:6,8:1-8:8" error-diagnostics="CS8754@7:1-7:6,CS8716@8:1-8:8"}
// 通常のメソッドとしてなら呼べる。
Ex.M(new());      // new object()
Ex.M(default); // null
...
```csharp {title="ターゲットからの型判定がかかるような例では拡張メソッドは呼べない" error-ranges="8:1-8:2,9:1-9:7" error-diagnostics="CS1929@8:1-8:2,CS1929@9:1-9:7"}
...
```csharp {title="ラムダ式のときは自然な型に対しても拡張メソッド呼び不可" error-text="(() =&gt; { }).M" error-diagnostics="CS0023@7:1-7:14"}
// これは行ける。
