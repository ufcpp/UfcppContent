---
title: "リンク集（C#）"
source_url: "https://ufcpp.net/study/csharp/appendix/link/"
content_type: "Article"
published_at: "2015-05-06T14:13:51"
updated_at: "2007-05-06T00:00:00"
tags: []
umbraco_id: 1382
parent_id: 1377
sort_order: 4
aliases:
  - "/csharp/appendix/link/"
  - "/csharp/link"
  - "/csharp/link.html"
  - "/study/csharp/link"
  - "/study/csharp/link.html"
---

# リンク集（C#）

##<a id="sec-generated-title-1"></a> <a id="community"></a>Community
[.NET/C# 初心者のためのメーリングリスト](http://www.egroups.co.jp/group/dotnet-csharp)
: [
          	[![Yahoo!グループ .NET/C#](../../../../assets/media/ufcpp2000/banner/logo_dotnet_csharp.gif)](../../../../assets/media/ufcpp2000/banner/logo_dotnet_csharp.gif)

        ](http://www.egroups.co.jp/group/dotnet-csharp).NET/C#用の初心者向け日本語メーリングリストです。 運営側の人間として僕も参加させていただいています。 掲示板形式でもアクセス可能です。

[INETA Japan](http://www.ineta.jp/)
: [
          	[![INETA Japan](../../../../assets/media/ufcpp2000/banner/logo_inetaj_m.gif)](../../../../assets/media/ufcpp2000/banner/logo_inetaj_m.gif)

        ](http://www.ineta.jp/)Microsoft .NET に興味を持つ開発者コミュニティの連携と活動の支援を行うことを目的とする組織です。 Microsoft sponsored。



##<a id="sec-generated-title-2"></a> <a id="csharp"></a>C
[MSDN Online .Net Developer Center](http://www.microsoft.com/japan/msdn/netframework/)
: マイクロソフトの.Netフレームワーク公式ホームページ。.Netフレームワークはここからダウンロードすることができます。

[SharpDevelop](http://icsharpcode.net/OpenSource/SD/default.asp)
: オープンソースなC#統合開発環境。 ソース付きです。

[C# Programming](http://uchukamen.com/)
: C# 初心者のためのヒント、チップス等が載っています。

[翔ソフトウェア](http://www.shos.info/)
: C/C++/C# による開発や、オブジェクト指向技術、そして XP 関連を中心に、 ソフトウェア開発のための技術情報をご紹介しています。

[WisdomSoft](http://wisdom.sakura.ne.jp/)
: 非常に多彩な言語・API に関する入門記事が揃っています。

[WoodenSoldier Software](http://www.woodensoldier.info/)
: C# に関する TIPS を非常に多く取り揃えています。

[C# と VB.NET の入門サイト](http://jeanne.wankuma.com/)
: 名前どおり、C# と VB.NET の入門サイトです。 サンプルが充実しています。

[AILight](http://www.ailight.jp/)
: C# や .NET Framework に関して、いろいろ研究していらっしゃいます。

[独学C#](http://www12.plala.or.jp/dokugaku/)
: C# の入門講座、リファレンスや、 C# で作成したフリーウェアのソースを公開しています。

[C#Note](http://www.geocities.jp/takamori700/index.htm)
: 趣味でMicrosoft Visual C# .NETを勉強している人のためのC#入門サイトです。

[.NET のつぼ](http://park16.wakwak.com/~mimi/dotnet/)
: C#, VB.NET による .NET プログラミングの逆引きマニュアルです。

[C#を攻略しよう](http://www.kisoplus.com/)
: Visual C# をサンプルプログラムを交えながら解説しているサイト。



##<a id="sec-generated-title-3"></a> <a id="aspdotnet"></a>ASP.net
[ASP.NET Developer Center](http://www.microsoft.com/japan/msdn/asp.net/)
: 日本語公式サイト。

[ASP.NET クイック スタート チュートリアル](http://ja.gotdotnet.com/quickstart/aspplus/)
: Microsoft が提供するチュートリアル。 Microsoft はドキュメントの多言語対応に結構力をいれているので、 日本語的にも割りと安心して読めると思います。

[＠IT 連載　 プログラミングASP.NET](http://www.atmarkit.co.jp/fdotnet/aspnet/index/index.html)
: こちらは＠ITの記事。
