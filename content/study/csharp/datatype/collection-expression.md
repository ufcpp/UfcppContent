...
```csharp {title="要素の型からの型推論" error-text="new[] { 1, 2, 3 }" error-diagnostics="CS0029@2:7-2:24"}
byte[] array // byte[] 型
...
```csharp {error-ranges="3:9-3:10,3:12-3:13,3:15-3:16,3:19-3:20" error-diagnostics="CS1525@3:9-3:10,CS1002@3:9-3:10,CS1513@3:12-3:13,CS1002@3:12-3:13,CS1513@3:15-3:16,CS1002@3:15-3:16,CS1002@3:19-3:20"}
int[] array; // 宣言
...
```csharp {title="配列とコレクションの切り替えが難しい" error-ranges="10:12-10:29" error-diagnostics="CS8752@10:12-10:29,CS1061@10:20-10:21,CS1061@10:23-10:24,CS1061@10:26-10:27"}
#if WPF
...
```csharp {title="参照型に対する stackalloc はエラーに" error-text="stackalloc[] { &quot;abc&quot; }" error-diagnostics="CS0208@1:21-1:43"}
...
```csharp {title="2つの {} 初期化子" error-ranges="2:46-2:47,2:49-2:50,2:52-2:53,14:5-14:13" error-diagnostics="CS0747@2:46-2:47,CS0747@2:49-2:50,CS0747@2:52-2:53,CS0747@14:5-14:13"}
...
```csharp {title="null があり得る foreach" warning-ranges="7:26-7:30" warning-diagnostics="CS8602@7:26-7:30"}
Print([1, 2, 3]);
Print(null);
...
```csharp {title="C# 12 時点では型決定できない var" error-text="[1, 2, 3]" error-diagnostics="CS9176@1:12-1:21"}
var list = [1, 2, 3];
```
...
```csharp {title="具象型同士は同列" error-ranges="1:3-1:4" error-diagnostics="CS0121@1:3-1:4"}
A.M([]); // コンパイル エラー
...
```csharp {title="派生関係のないインターフェイスは同列" error-ranges="1:3-1:4" error-diagnostics="CS0121@1:3-1:4"}
A.M([]); // コンパイル エラー
...
```csharp {title="拡張メソッドからの型推論" error-ranges="2:9-2:15" error-diagnostics="CS9176@2:9-2:15"}
...
```csharp {title="非ジェネリック コレクション" error-text="[&quot;a&quot;, 2, null]" error-diagnostics="CS9174@3:17-3:31"}
