...
```csharp {title="不明瞭でオーバーロード解決できない例" error-ranges="19:9-19:10"}
using System;
...
```csharp {title="型推論が働かなくなる例" error-ranges="12:9-12:10,15:9-15:10,21:9-21:10"}
using System;
...
```csharp {title="ラムダ式とオーバーロード解決" error-ranges="16:9-16:10"}
using System;
...
```csharp {title="同名の静的メソッドとインスタンス メソッド" error-ranges="16:9-16:18,20:9-20:24"}
using System;
...
```csharp {title="静的メソッドかインスタンス メソッドかでオーバーロード解決" error-ranges="33:9-33:10"}
using System;
...
```csharp {error-ranges="33:15-33:16"}
using System;
...
```csharp {title="制約違いのオーバーロードは不可" error-ranges="4:13-4:14"}
// 以下の2つは呼び分けできるようになった。
...
```csharp {title="オーバーロードできないメンバーに OverloadResolutionPriority を付けるとコンパイラーに怒られる" error-ranges="16:6-16:35,19:6-19:35,22:6-22:35,25:6-25:35" error-diagnostics="CS9262@16:6-16:35,CS9262@19:6-19:35,CS9262@22:6-22:35,CS9262@25:6-25:35"}
using System.Runtime.CompilerServices;
...
```csharp {error-ranges="2:3-2:4" error-diagnostics="CS0121@2:3-2:4"}
...
```csharp {title="ただし、実際にやってみるとうまくいかない(当然)" error-text="Select" error-diagnostics="CS0121@3:7-3:13"}
...
```csharp {title="OverloadResolutionPriority を付けたことで他のクラスのメンバーと競合するようになる例" error-ranges="6:4-6:5" error-diagnostics="CS0121@6:4-6:5"}
using System.Runtime.CompilerServices;
...
```csharp {title="戻り値だけが違うオーバーロードの追加はできない" error-ranges="7:35-7:41" error-diagnostics="CS0111@7:35-7:41"}
class C
{
