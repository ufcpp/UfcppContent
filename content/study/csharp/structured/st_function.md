...
```csharp {title="引数名違いのオーバーロードは無理" error-ranges="7:13-7:14"}
// F は、引数の型が違うので大丈夫
static void F(int x) { }
...
```csharp {title="戻り値違いのオーバーロードも無理" error-ranges="3:15-3:16"}
...
```csharp {title="in, ref, out の扱い" error-ranges="2:6-2:7,5:6-5:7,8:6-8:7"}
void F(ref int x) { }
void F(in int x) { }
