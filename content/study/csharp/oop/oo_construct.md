...
```csharp {title="初期化子内ではインスタンス メソッドを呼んではいけない" error-ranges="4:30-4:31" error-diagnostics="CS0236@4:30-4:31" warning-ranges="6:17-6:28" warning-diagnostics="CS0649@6:17-6:28"}
class C
{
...
```csharp {title="型名省略前からダメなものはダメ" error-ranges="7:22-7:28,14:11-14:16,15:13-15:18"}
...
```csharp {title="プライマリ コンストラクターを呼ばないとエラー" error-ranges="8:12-8:18" error-diagnostics="CS8862@8:12-8:18"}
class Person(string name, int age)
{
    public string Name = name;
    public int Age = age;
...
```csharp {title="プライマリ コンストラクター持ちのクラスとレコード" warning-ranges="1:17-1:18,1:24-1:25" warning-diagnostics="CS9113@1:17-1:18,CS9113@1:24-1:25"}
...
```csharp {title="2重にフィールド生成がかかってしまう例" warning-ranges="7:30-7:31" warning-diagnostics="CS9124@7:30-7:31"}
class C(int x)
{
    // こちらは「キャプチャ」。
    public int X1 => x;
...
```csharp {title="一度 readonly フィールドで受け取る" error-ranges="12:25-12:27" error-diagnostics="CS0191@12:25-12:27" warning-ranges="4:31-4:32" warning-diagnostics="CS9124@4:31-4:32"}
partial class C(int x)
{
    // フィールドで受け取る。
    private readonly int _x = x;
