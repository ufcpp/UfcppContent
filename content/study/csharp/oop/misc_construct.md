...
```csharp {title="基底クラスのコンストラクターが呼ばれる"}
// コンストラクター呼び出し。
new D();
...
```csharp {title="コンストラクター内で基底クラスのメンバーを使用"}
var d = new D();
...
```csharp {title="基底クラスのコンストラクターで仮想メソッドを呼ぶ"}
...
```console {title="C++ で同様のプログラムを書いて実行させた結果"}
base
```
...
```csharp {title="初期化されるまえにメンバー変数を参照してしまう"}
// コンストラクター呼び出し。
new D("derived");
...
```csharp {title="プライマリ コンストラクターを使うと初期化タイミングが早い"}
// コンストラクター呼び出し。
new D("derived");
