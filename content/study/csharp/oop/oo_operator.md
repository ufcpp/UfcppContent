...
```csharp {title="値渡しはOK。参照渡し(ref引数)はNG" error-ranges="12:36-12:37"}
class Complex
{
    public double X;
    public double Y;
...
```csharp {title="+= だけあっても + は使えない" error-text="x + 10" error-diagnostics="CS0019@7:5-7:11"}
var x = new X(5);
...
```csharp {title="後起きインクリメントはダメ" error-ranges="7:9-7:12" error-diagnostics="CS0023@7:9-7:12"}
var x = new X(1);
