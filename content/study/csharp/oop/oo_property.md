...
```csharp {title="get-only なプロパティ" highlight-ranges="5:24-5:28,6:24-6:28"}
using System;
...
```csharp {highlight-ranges="3:31-3:37,4:32-4:37"}
class Point
{
...
```csharp {title="init アクセサー" highlight-ranges="3:29-3:33,4:29-4:33"}
class Complex
{
...
```csharp {title="get-only プロパティはオブジェクト初期化子を使えない" error-ranges="1:21-1:22,1:28-1:29"}
...
```csharp {title="init-only プロパティならオブジェクト初期化子を使える" highlight-ranges="5:25-5:29,6:25-6:29"}
var p = new Point { X = 1, Y = 2 };
...
```csharp {title="名前指定初期化はクラス定義側が楽" warning-ranges="12:19-12:20" warning-diagnostics="CS8618@12:19-12:20"}
...
```csharp {title="とにかく「初期化を明示させたい」" error-ranges="4:14-4:15" error-diagnostics="CS9035@4:14-4:15"}
...
```csharp {title="派生と required" error-ranges="20:25-20:26" error-diagnostics="CS9030@20:25-20:26"}
abstract class A
{
...
```csharp {title="インターフェイスには required を使えない" error-text="X" error-diagnostics="CS0106@4:18-4:19"}
interface I
{
    // エラー。
...
```csharp {title="required メンバーのアクセシビリティの制限" error-ranges="10:28-10:30,11:27-11:29,12:37-12:39,13:36-13:38,14:26-14:28" error-diagnostics="CS9032@10:28-10:30,CS9032@11:27-11:29,CS9032@12:37-12:39,CS9032@13:36-13:38,CS9032@14:26-14:28"}
internal class A
{
...
```csharp {title="自称 SetsRequiredMembers" warning-ranges="14:12-14:13" warning-diagnostics="CS8618@14:12-14:13"}
using System.Diagnostics.CodeAnalysis;
...
```csharp {title="ref 付きのプロパティは field-backed プロパティにできない" error-ranges="4:20-4:21,7:20-7:21" error-diagnostics="CS8145@4:20-4:21,CS8145@7:20-7:21" warning-ranges="10:21-10:23" warning-diagnostics="CS9265@10:21-10:23"}
ref struct RefField
{
...
```csharp {title="field は文脈キーワード" warning-ranges="4:17-4:22,15:21-15:26,20:7-20:12" warning-diagnostics="CS0169@4:17-4:22,CS9258@15:21-15:26,CS8981@20:7-20:12"}
class A
{
    // これは普通にフィールド。
    private int field;
...
```csharp {title="field キーワードの追加は破壊的変更" warning-ranges="3:17-3:22,8:21-8:26" warning-diagnostics="CS0649@3:17-3:22,CS9258@8:21-8:26"}
class A
{
    private int field;
...
```csharp {title="get の解析結果を踏まえて set をフロー解析" warning-ranges="9:24-9:29,13:19-13:20,40:24-40:29" warning-diagnostics="CS8600@9:24-9:29,CS9264@13:19-13:20,CS8600@40:24-40:29"}
class Nullability
{
    public string X
    {
