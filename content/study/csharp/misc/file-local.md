...
```csharp {title="file は型のみ" error-ranges="3:14-3:16,5:14-5:15" error-diagnostics="CS0106@3:14-3:16,CS0106@5:14-5:15"}
class A
{
    file int _x;
...
```csharp {title="アクセシビリティとの併用不可" highlight-text="internal file" error-text="X" error-diagnostics="CS9052@1:28-1:29"}
...
```csharp {title="入れ子の型不可" highlight-text="file" error-text="NestedFileClass" error-diagnostics="CS9054@3:16-3:31"}
class A
{
