...
```csharp {title="データベース テーブルの Id, Name 列からのプロパティ生成"}
class Entity
{
...
```csharp {title="ツール生成物をもし手で書き換えたとすると…"}
class Entity
{
    private int _id;
...
```csharp {title="残念ながら、手書きで書き換えた分は紛失する"}
class Entity
{
    public int Id { get; set; }
    public string? Name { get; set; }
...
```csharp {title="partial を使えば、別ファイルでクラスに処理を足せる"}
partial class Entity
{
    private bool _changed;
    public void Flush() => _changed = false;
...
```csharp {title="構造体、インターフェイスにも partial"}
partial struct S { }
partial struct S { }
...
```csharp {title="WPF で、XAML から生成されるコード例"}
//------------------------------------------------------------------------------
// <auto-generated>
...
```csharp {title="ツール生成で partial メソッドを生成"}
partial class Entity
{
    private int _id;
...
```csharp {title="partial メソッドに実装を足す例"}
partial class Entity
{
...
```csharp {title="新旧・部分メソッド" highlight-ranges="sha256:cf1165d0fc54a5db975165a51716c6b8ab9cb53ecebb95e56603d60f82e28fe4;30:5-30:12"}
// (1) ツールが事前に生成する想定のコード
...
```csharp {title="abstract の get; set;"}
interface I
{
