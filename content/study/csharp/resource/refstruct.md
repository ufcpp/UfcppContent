...
```csharp {error-ranges="2:5-2:10,7:13-7:22"}
...
```csharp {title="戻り値に返せるかどうか" error-ranges="8:12-8:13,30:12-30:14"}
// 引数で受け取ったものは戻り値で返せる
...
```csharp {title="Span&gt;T&lt;とref T" error-ranges="8:16-8:17"}
...
```csharp {title="readonly修飾とref構造体" error-ranges="25:9-25:24"}
using System;
...
```csharp {title="ref構造体は stack-only" error-ranges="6:5-6:10,9:24-9:35,14:39-14:40,17:9-17:18,27:9-27:14,35:22-35:27,38:19-38:24,41:30-41:35,42:20-42:25,45:14-45:23"}
using System;
...
```csharp {error-ranges="3:13-3:15,8:13-8:15" error-diagnostics="CS9059@3:13-3:15,CS9059@8:13-8:15"}
class A
{
...
```csharp {title="✔「どこを参照するか変更」と✔「参照先の値の変更」" highlight-ranges="10:12-10:15"}
scoped var a = new A();
...
```csharp {title="✔「どこを参照するか変更」と✖「参照先の値の変更」" highlight-text="ref readonly" error-ranges="6:1-6:2" error-diagnostics="CS8331@6:1-6:2"}
scoped var a = new A();
...
```csharp {title="✖「どこを参照するか変更」と✔「参照先の値の変更」" highlight-text="readonly ref" error-ranges="7:1-7:2" error-diagnostics="CS0191@7:1-7:2"}
int x0 = 0;
...
```csharp {title="✖「どこを参照するか変更」と✖「参照先の値の変更」" error-ranges="7:1-7:2,9:1-9:2" error-diagnostics="CS0191@7:1-7:2,CS8331@9:1-9:2"}
int x0 = 0;
...
```csharp {title="ローカル変数への参照は外に漏らせない" error-ranges="4:16-4:17" error-diagnostics="CS8168@4:16-4:17"}
static ref int M()
{
...
```csharp {title="エスケープ阻止のため、多段に追う必要あり" error-ranges="6:16-6:17" error-diagnostics="CS8157@6:16-6:17"}
static ref int M()
{
    int x = 123; // メソッド内の変数はメソッド抜けると消える。
    ref var y = ref x;
...
```csharp {title="参照がいろんなところに伝搬する例" error-ranges="9:14-9:20,10:14-10:15,11:14-11:17" error-diagnostics="CS8352@9:14-9:20,CS8352@10:14-10:15,CS8352@11:14-11:17"}
...
```csharp {title="(仮定的なコードで) 参照の伝搬をすべて明示" highlight-ranges="18:21-18:23,20:21-20:23,20:35-20:37,20:54-20:56" error-ranges="12:14-12:17"}
static void M(out Span<int> result)
{
    int x = 123;
...
```csharp {title="何もつけない: ref T は return-only、ref 構造体は unscoped" error-ranges="21:38-21:48" error-diagnostics="CS9079@21:38-21:48"}
ref struct Default
{
    private ref int _x;
    private Span<int> _y;
...
```csharp {title="scoped 修飾子を付けた場合" error-ranges="13:40-13:50,14:55-14:56,15:45-15:55,17:47-17:48,18:56-18:57,19:53-19:54" error-diagnostics="CS8374@13:40-13:50,CS9075@14:55-14:56,CS8374@15:45-15:55,CS8352@17:47-17:48,CS8352@18:56-18:57,CS8352@19:53-19:54"}
ref struct Scoped
{
    private ref int _x;
    private Span<int> _y;
...
```csharp {error-ranges="17:22-17:33,18:34-18:45,19:26-19:37,20:26-20:37" error-diagnostics="CS9063@17:22-17:33,CS9063@18:34-18:45,CS9063@19:26-19:37,CS9063@20:26-20:37"}
...
```csharp {title="unscoped な挙動" error-ranges="8:5-8:31" error-diagnostics="CS8350@8:5-8:31,CS8352@8:21-8:30"}
...
```csharp {title="this は scoped 扱い" error-ranges="7:35-7:39,9:34-9:36" error-diagnostics="CS8170@7:35-7:39,CS8170@9:34-9:36"}
using System.Diagnostics.CodeAnalysis;
...
```csharp {title="インターフェイスを実装できるようになったのに、インターフェイスに代入できない" error-text="new S()" error-diagnostics="CS0029@1:18-1:25"}
...
```csharp {title="allows ref struct な型の変数はボックス化できない" error-ranges="4:16-4:17,5:23-5:24,6:23-6:24" error-diagnostics="CS0103@4:16-4:17,CS0103@5:23-5:24,CS0103@6:23-6:24"}
static void M<T>() where T : allows ref struct
{
...
```csharp {title="allows ref struct なジェネリック メソッドを介して、ref 構造体のインターフェイス実装を呼ぶ" error-ranges="4:18-4:19" error-diagnostics="CS0029@4:18-4:19"}
...
```csharp {title="ref 構造体は IEnumerable と相性がよくない" error-text="new Enumerator(this)" error-diagnostics="CS0029@18:46-18:66"}
