...
```csharp {title="式中の任意の場所に stackalloc" error-ranges="36:13-36:23"}
using System;
...
```csharp {title="拡張メソッド呼び出しができなかった例(C# 14 で解決)" error-ranges="1:1-1:11,2:1-2:3" error-diagnostics="CS1929@1:1-1:11,CS1929@2:1-2:3"}
new int[0].M();
"".M();
...
```csharp {title="ユーザー定義の型変換ができなかった例(C# 14 で解決)" error-ranges="1:5-1:15,2:5-2:7" error-diagnostics="CS1503@1:5-1:15,CS1503@2:5-2:7"}
X.M(new int[0]);
X.M("");
...
```csharp {title="ジェネリック型推論ができなかった例(C# 14 で解決)" error-ranges="1:3-1:4" error-diagnostics="CS0411@1:3-1:4"}
X.M(new int[0]);
...
```csharp {title="IEnumerable と ReadOnlySpan の解決ができなかった例(C# 14 で解決)" error-ranges="3:11-3:12" error-diagnostics="CS0121@3:11-3:12"}
int[] data = [1, 2, 3];
