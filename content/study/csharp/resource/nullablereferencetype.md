...
```csharp {title="opt-in した瞬間に警告" warning-ranges="8:20-8:24,9:27-9:28"}
using System;
...
```csharp {title="null 許容コンテキストの切り替え例" warning-ranges="6:12-6:16"}
...
```csharp {title="null 許容参照型はフロー解析で null チェックをしてる" warning-ranges="14:19-14:20"}
// null 許容で宣言されていても、
string? s;
...
```csharp {title="フロー解析は分岐もちゃんと調べる" warning-ranges="9:23-9:24"}
...
```csharp {title="警告の出方" warning-ranges="13:25-13:29,18:27-18:28"}
using System;
...
```csharp {title="メンバー アクセスを持って null チェック扱い" warning-ranges="5:23-5:24"}
#nullable enable
void M(string? x)
{
    // null チェックせずに使ったので警告。
...
```csharp {title="他の変数との比較で null チェック" warning-ranges="12:27-12:28"}
...
```csharp {title="非 null なフィールド・プロパティを初期化しないと警告が出る" warning-ranges="3:19-3:20,4:19-4:20"}
class A
{
    public string X;
...
```csharp {title="初期か忘れ警告" warning-ranges="3:19-3:20,6:12-6:13"}
class A
{
    public string X;
...
```csharp {title="オーバーロードの可否" error-ranges="4:6-4:7"}
#nullable enable
...
```csharp {title="null チェック後の挙動" error-ranges="17:25-17:31"}
#nullable enable
// 参照型の場合
void M(string? x)
{
...
```csharp {title="循環参照があるとき、コンストラクターでは非 null 保証ができない" warning-ranges="4:23-4:30"}
class PairedNode
{
    // このプロパティに対する警告が消せない。
...
```csharp {title="ReferenceEquals でも等価チェックになるはずなのに" warning-ranges="7:27-7:28"}
void M(string x, string? y)
{
...
```csharp {title="! を付けて強制非 null 扱い" highlight-ranges="8:55-8:60,18:32-18:33"}
#nullable enable
using System;
...
```csharp {title="制約なしの型引数 T に対して T? は使えない" error-ranges="5:12-5:13"}
#nullable enable
class Generic<T>
{
...
```csharp {error-ranges="8:12-8:13,15:12-15:13"}
#nullable enable
using System;
...
```csharp {title="notnull 制約" warning-ranges="18:41-18:45,19:41-19:48"}
#nullable enable
...
```csharp {title="notnull を付けても T? とは書けない" error-ranges="7:5-7:7,7:15-7:19,8:11-8:13"}
#nullable enable
...
```csharp {title="アノテーション属性の影響はメソッド内部には及んでなかった(リリース当初)" warning-ranges="9:26-9:30,17:27-17:35"}
#nullable enable
using System;
using System.Diagnostics.CodeAnalysis;
...
```csharp {title="間接的な初期化をしているフィールド" warning-ranges="5:12-5:25"}
class DeflateStream
{
